 (function () { function r(e, n, t) { function o(i, f) { if (!n[i]) { if (!e[i]) { var c = "function" == typeof require && require; if (!f && c) return c(i, !0); if (u) return u(i, !0); var a = new Error("Cannot find module '" + i + "'"); throw a.code = "MODULE_NOT_FOUND", a } var p = n[i] = { exports: {} }; e[i][0].call(p.exports, function (r) { var n = e[i][1][r]; return o(n || r) }, p, p.exports, r, e, n, t) } return n[i].exports } for (var u = "function" == typeof require && require, i = 0; i < t.length; i++)o(t[i]); return o } return r })()({
   1: [function (require, module, exports) {
     ...
     //import Consts from "./../../../gulp/consts/const";
     ...
     function getHead() {
       if (vm.newsIntroduction) {
         var Summary = vm.newsIntroduction.Summary, Id = vm.newsIntroduction.NoticeId, Title = vm.newsIntroduction.Title, ShareIcon = shareIcon;
       }
       else {
         var Summary = '', Id = 0, Title = '暂无内容，无法进行分享操作', ShareIcon = "";
       }
       HeaderObj = {
         ...
         rightIcon: [
           {
             ...
             share: {
               ...
               shareModel: {
                 ...
                 title: Title,
                 content: Summary,
                 imgUrl: vm.newsIntroduction ? vm.newsIntroduction.TitleImg : "",
                 shareUrl: pageUrl
               }
             }
           }
     ...
     function preSetImg(txt) {
       let screenWidth = window.screen.width - 40;
       let newContent = txt.replace(/<img[^>]*>/gi, function (match, capture) {
         let regWidth = /data-width=\s*?([‘"])([\s\S]*?)\1/ig;
         regWidth.test(match);
         let width = RegExp.$2;
         let regHeight = /data-height=\s*?([‘"])([\s\S]*?)\1/ig;
         regHeight.test(match);
         let height = RegExp.$2;
         ...
         if (width && height) {
           ...
           height = Math.round(screenWidth * rate) + 'px';
         }
         let heightStr = 'style="border:none;background:#f7f7f7;width:100%;height:' + height + ';"';
         return match.replace(regHeight, heightStr)
       })
       return newContent
     }

     function handleImgs(imgs, n) {
       if (imgs.length < 1) return;
       if (clickImageArray.length > 0) clickImageArray = [];
       for (var i = 0; i < imgs.length; i++) {
         var cur = $("[data-viewer]").eq(i);
         var cur_src = $(cur).attr("src");
         $(cur).attr({
           "data-num": i,
           "data-src": cur_src,
           "data-preview-group": n,
           "data-preview-src": "",
           "class": "zoom",
           "id": "img_" + i,
         });
         ...
         clickImageArray.push(cur_src);
         var img = new Image();
         imgList[i] = img;
         imgList[i].src = cur_src;
       }
     }
     function handleVideos(videos) {
       ...
       for (var i = 0; i < videos.length; i++) {
         var cur = $(videos).eq(i);
         $(cur).removeAttr("width height");
         var curWidth = $(cur).width();
         var curHeight = curWidth * 3 / 4;
         $(cur).height(curHeight);
         $(cur).wrap("<div></div>");
         var parent = $(cur).parent();
         $(parent).css("height", $(cur).css("height"));
         $(parent).addClass("video-box");
         var html = '<div class="layer"></div>' +
           '<img class="play-btn" src = "' + hosts.imgHost + '/upload/files/master/2020/04/21/141154528.png" />';
         $(parent).append(html);
         $(cur).attr("poster") ? $(cur).siblings(".layer").css("background-image", `url(${$(cur).attr("poster")})`) : "";
       }
       var timer = 0;
       $(".news_content").on("tap", ".play-btn", function () {
         var newTime = Date.now();
         if (newTime - timer > 500) {
           timer = newTime;
           var that = $(this).siblings("video").get(0);
           //全屏播放
           var videoUrl = $(that).attr("src");
           var videoDuration = Number($(that).attr("duration"));
           var videoArgs = {
             "videoUrl": videoUrl,
             "videoDuration": videoDuration ? videoDuration : 0
           }
           bk.booksInit("onClickWebVideo", videoArgs);
         }
       });
     }
     ...
     windows_1["default"].loadNewsUrl = function (url) {      
       pageUrl = url.replace("officialdetails", "officialshares");
     };
     windows_1["default"].loadData = function (data) {
       //头部信息
       if (data.Logo == null || data.Logo == "") {
         data.Logo = hosts.imgHost + "/upload/files/master/2020/04/20/123112715.png";
       }
       //原生头部获取信息
       vm.newsIntroduction = data;
       getHead();
       if (data.Title && data.Title != "") {
         var topHtml = '<div class="new_title">' +
           '<p>' + data.Title + '</p>' +
           '</div>' +
           '<div class="news_attach">' +
           '<img class="news_platform_logo" id="news_platform_logo" src="' + data.Logo + '"/>' +
           '<span class="new_platform_name">汇查查 | </span>' + (data.AddTime ? '<span class="publish_time">' + globalTimeLimit_1["default"](data.AddTime) + '</span>' : '') +
           '<div class="clk-num">浏览量：' + resultData(data.BrowseCount) + '</div>';
         '</div>';
         $(".main_top").html(topHtml);
         //主要新闻信息
         var contentHtml = "";
         //新闻详情内容处理
         var contentData = void 0;
         if (data.Content && data.Content != "") {
           var str = data.Content ? data.Content.replace(/\\n+/ig, '</p><p>') : "";
           ...
           // }
           // contentData = TextTransformation2.MobiletoWeb(mcontMsg, imgList, 0);
           contentData = preSetImg(txt);
         }
         //主要新闻渲染
         contentHtml += '<div class="content_text" >' + contentData + '</div>';
         $(".news_content").html(contentHtml);
         
         //处理图片集
         var preImgs = $("[data-viewer]");
         handleImgs(preImgs, 1);
         //新闻中图片懒加载
       ...
       GlobalTimeLimit.prototype.getTimeString = function () {
         ...
         if (this.sameDay()) {
           ...
           if (distanceStamp < 60 * 60 * 1000) {
             ...
             //let hours = this.systemHour - this.addHour;
             var hours = Math.floor(distanceStamp / 3600000);
             return hours + "\u5C0F\u65F6\u524D";
           }
         }
     ...
     function MobileToWeb(html, imgList, index) {
       ...
       //var t = html.Content ? html.Content.replace(/(\\n )+/g, '<br/>') : "";
       var t = html.Content;
         ...
         if (a.length > 0) {
           for (var i = 0; i < a.length; i++) {
             if (t.indexOf("{img" + i + "}") >= 0) {
               ...
               t = t.replace("{img" + i + "}", '<img class="zoom" data-num="' + (i + index) + '"  id="img_' + i + '" data-src="' + n + '" data-preview-src="" data-preview-group="1" style="width:100%;height:' + setHight + ';border:none;background:#f7f7f7"/>');
               var img = new Image();
               imgList[i] = img;
               imgList[i].src = n;
             }
       ...
       for (var i in imgList) {
         (function (i) {
           imgList[i].onload = function () {
            var src = $("#img_" + i).data("src");
            $("#img_" + i).attr("src", src).css({ "opacity": 1 });
           };
         })(i);
       }
     }
