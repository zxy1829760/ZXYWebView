...
//        console.log(hosts)
    ...
    function getHead() {
      if (vm.newsIntroduction) {
        var Summary = vm.newsIntroduction.Summary, Id = vm.newsIntroduction.NewsId, SmallImage = vm.newsIntroduction.TitleImages.Small, Title = vm.newsIntroduction.Title, ShareIcon = shareIcon;
      }
      else {
        var Summary = '', Id = 0, SmallImage = "", Title = '暂无内容，无法进行分享操作';
        ShareIcon = "";
      }
      HeaderObj = {
    ...
    windows_1["default"].loadNewsUrl = function (url) {
      pageUrl = url.replace("details", "shares");
    };
    //ios和android提供数据(data为所需的数据)
    windows_1["default"].loadData = function (data) {
      if (data && data != '') {
//          console.log(hosts.imgHost)
        //原生头部获取信息
        var mainData = data.NewsDetail;
        vm.newsIntroduction = mainData;
        getHead();
        if (mainData.Title && mainData.Title != "") {
          //头部信息
          var topHtml = '<div class="new_title">' +
            '<p>' + mainData.Title + '</p>' +
            '</div>' +
            '<div class="news_attach">' +
            '<img class="news_platform_logo" src = "' + hosts.imgHost + '/upload/files/master/2020/04/20/123349106.png" >' +
            '<span class="new_platform_name" > 酷汇讯 | </span>' +
            (mainData.AddTime ? '<span class="publish_time">' + globalTimeLimit_1["default"](mainData.AddTime) + '</span>' : '') +
            '</div>';
          $(".main_top").html(topHtml);
         console.log($(".news_platform_logo").attr("src"));
          ...
          //新闻详情内容处理
          var contentData = '';
          if (mainData.MContent && data.MContent != "") {
            //如果内容开始是图片，将图片置为空（此时图片与banner图相同）
            var changeStr = JSON.stringify(mainData.MContent);
            var str = changeStr ? changeStr.replace(/\\n+/ig, '</p><p>') : "";
          ...
          //主要新闻渲染
          contentHtml += '<div class="news_text" >' + contentData + '</div>';
      ...
      GlobalTimeLimit.prototype.getTimeString = function () {
        ...
        if (this.sameDay()) {
          ...
          if (distanceStamp < 60 * 60 * 1000) {
            ...
            // var hours = this.systemHour - this.addHour;
            // return hours + "\u5C0F\u65F6\u524D";
        ...
        if (a.length > 0) {
          for (var i = 0; i < a.length; i++) {
            if (t.indexOf("{img" + i + "}") >= 0) {
              ...
              if (jug == undefined) {
                setHight = Math.round(screenWidth * 0.666) + 'px';
              }
              ...
              imgList[i] = img;
              imgList[i].src = n;
              //t = t.replace("{img" + i + "}", '<img src="' + n + '"data-preview-src="" data-preview-group="1">');
            }
